...
(function() {
	...
	class Person {
		...
		get children() {
			let hasChildren = this.relationships.family.children.length;

			if (hasChildren) {
				log('in get children', this.relationships.family.children);
			}
			return (hasChildren) ? this.relationships.family.children: false;
		}

		set children(child) {
			!child && logER('no child passed to set children');
			log('in set children with', child);
			this.relationships.family.children.push(child);
		}
